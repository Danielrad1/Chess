...
public class Rook extends Piece {
    ...
    private final static int[] candidate_move_vectors = {1, -1, -8, 8};

    public Rook(final Colour colour, final int position) {
        super(Piece_Type.ROOK, position, colour, true);
    }
    public Rook(final Colour colour, final int position, final boolean is_first_move) {
        super(Piece_Type.ROOK, position, colour, is_first_move);
    }
    ...
    public Rook Move_piece(Move move) {
        return new Rook(move.get_moved_piece().get_piece_colour(), move.Get_Destination());
    }

    @Override
    public int locationBonus() {
        return this.piece_colour.rookBonus(this.piece_position);
    }
    ...
    public static boolean is_first_column_exception(final int position, final int offset){
        return BoardUtils.is_a_file(position) && (offset == -1);
    }
    public static boolean is_eighth_column_exception(final int position, final int offset){
        return BoardUtils.is_h_file(position) && (offset == 1);
    }
}
